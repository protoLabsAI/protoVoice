...
export class VoiceOrb {
  constructor(container, { preset = 'Aurora' } = {}) {
    ...
    // Authoritative state (idle|listening|thinking|speaking) set via
    // setState() by an outside driver (RTVI events). When non-null, it
    // overrides the audio-envelope-derived state in _tick().
    this._externalState = null;
  ...
  /**
   * Drive the 4-state machine from the outside (RTVI events). Pass
   * null to release control and fall back to audio-envelope derivation.
   */
  setState(nextState) {
    this._externalState = nextState ?? null;
  }

  /** Alias for destroy() — React effects expect a dispose method. */
  dispose() {
    this.destroy();
  }
  ...
  _tick() {
    ...
    // ---- State machine --------------------------------------------------
    // Prefer the RTVI-driven authoritative state when available; fall
    // back to audio-envelope inference only when nothing has set it
    // (e.g. pre-connect screen, or if integration drops).
    const nextState = this._externalState ?? this._deriveState(bot, user, nowMs);
